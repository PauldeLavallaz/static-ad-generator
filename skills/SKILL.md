...
Brand Research → Claude builds a Brand DNA document via web search
Prompt Generation → Claude fills 40 template prompts with brand-specific details
Image Generation → Python script fires prompts to Nano Banana 2 via FAL API
...
Reads prompts.json from the brand folder
Scans product-images/ for available reference images
For each prompt:
...
Downloads generated images to outputs/{template-number}-{template-name}/
Saves the prompt text alongside each image as prompt.txt
Generates an index.html gallery showing all generated ads
FAL API Details
...
REST API pattern (Python):
# Submit to queue
POST https://queue.fal.run/fal-ai/nano-banana-2
Headers: {"Authorization": "Key {FAL_KEY}", "Content-Type": "application/json"}
Body: {"input": {"prompt": "...", "aspect_ratio": "1:1", "num_images": 1, "output_format": "png"}}
...
Returns: {"url": "https://..."}  # Use this URL in image_urls
...
Run Phase 3 with --templates filter
Re-run with New Product (user says "now do it for [different SKU]")
Skip Phase 1 (Brand DNA already exists)
